# S3: Immune escape prevalence across TMB intervals after excluding mutation features
# This pan-cancer analysis removes tumors positive for selected mutation-based
# escape features and evaluates the remaining immune escape features across 20
# intervals spanning the central 90% of log10(TMB). Trends are assessed using a
# permutation-based Cochran-Armitage test and displayed as prevalence curves.
...
profile.trans.escape=profile.escape.binarization 	%>%		
    ...
    dplyr::reframe(
        Checkpoint.overexprs=any(CD274, CTLA4, PDCD1LG2, PDCD1, FGL1, LAG3, BTLA, TIGIT, HAVCR2, na.rm=TRUE),		
        Novel.Check.overexprs=any(CD47, ENTPD1, NT5E, na.rm=TRUE), 
...
# Non-mutation immune escape feature groups.
all.features.list=list(
    APMalt=c("HLA.LOH", "biallelic.B2M.inactivation"),
    Checkalt=c("CD274.deepAmp"),
    ActMalt=c("IFNG.pathway.HD", "CD58.HD"),
...
mut.features=c('all.APM.mut', 'IFNG.pathway.mut', 'IDH1.mut', 'CD58.mut')

# Exclude mutation-positive tumors and define TMB intervals.
data=profile.escape.binarization    %>%
    dplyr::left_join(profile.TMB %>% select(SampleID, TMB), by="SampleID")  %>%
    dplyr::filter(!if_any(all_of(mut.features), ~ .x == TRUE))

TMB.bucket=20
TMB.group.data=data       %>%        
    dplyr::filter(!is.na(TMB))     %>%        
    dplyr::reframe(SampleID=SampleID, TMB=TMB, log10_TMB=log10(TMB))     %>%
    dplyr::filter((log10_TMB>quantile(log10_TMB, 0.05)) & (log10_TMB<quantile(log10_TMB, 0.95)))    %>%
...
# Cochran-Armitage trend tests across ordered TMB intervals.
...
CA.data=Reduce(function(x, y)      dplyr::full_join(x, y, by='SampleID'), list(profile.escape.binarization, profile.trans.escape, TMB.group.data))
CA.data=CA.data %>%
...
CA.test.result$fdr_p_value=p.adjust(CA.test.result$p_value, method="BH")
...
pdf(file.path(Dir.output, "3.2.escape_feature_prevalence_across_TMB_groups_excluding_all_mutation_positive_samples.pan_cancer.pdf"), 7, 4)
plots=lapply(all.features.list, function(features){
    data=Reduce(function(x, y)      dplyr::full_join(x, y, by='SampleID'), list(profile.escape.binarization, profile.trans.escape, TMB.group.data))
    ...
    p1=ggplot(df1, aes(x=TMB.group, y=TMB.group.sam))+
            geom_col(alpha=0.4)+labs(x='', title=paste0('PanCancer (no mutation escape features)'), y='samples')+
            theme_test() +
    ...
    tmp2=CA.test.result        %>%
        dplyr::mutate(features=binary.var, z.statistic, fdr_p_value)
    df3=dplyr::full_join(tmp1, tmp2, by="features")     %>%
        na.omit()   %>%
        dplyr::mutate(label=case_when(
            z.statistic>0 & fdr_p_value<0.05 ~ "pos.cor",
            z.statistic<0 & fdr_p_value<0.05 ~ "neg.cor",
            TRUE ~ ""
        ))  %>%
        unique()
