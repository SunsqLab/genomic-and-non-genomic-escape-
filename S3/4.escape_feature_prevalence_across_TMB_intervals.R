# S3: Immune escape feature prevalence across tumor mutational burden intervals
# This script combines cancer-specific TCGA profiles, restricts the analysis to
# primary tumors, and divides the central 98% of log10(TMB) into 20 intervals.
# Feature prevalence trends are evaluated by permutation-based Cochran-Armitage
# tests for all cancers and after excluding CRC, STAD, and UCEC.
...
# TMB intervals for all cancers and for cancers excluding CRC, STAD, and UCEC.
...
all.CA.test.result=lapply(names(TMB.group.data), function(tmp.cancer){
    CA.data=Reduce(function(x, y)      dplyr::full_join(x, y, by='SampleID'), list(profile.escape.binarization, profile.trans.escape, TMB.group.data[[tmp.cancer]]))
    CA.data=CA.data %>%
    ...
    writeData(wb, sheet=sheet.name, CA.test.result, startRow=2, headerStyle=createStyle(textDecoration="bold"))
...
pdf(file.path(Dir.output, "3.1.escape_feature_prevalence_across_TMB_groups.pan_cancer.pdf"), 7, 4)
result=lapply(names(TMB.group.data), function(tmp.cancer){
    plots=lapply(all.features.list, function(features){
        data=Reduce(function(x, y)      dplyr::full_join(x, y, by='SampleID'), list(profile.escape.binarization, profile.trans.escape, TMB.group.data[[tmp.cancer]]))
        ...
        p1=ggplot(df1, aes(x=TMB.group, y=TMB.group.sam))+
                geom_col(alpha=0.4)+labs(x='', title=paste0('PanCancer ', tmp.cancer), y='samples')+
                theme_test() +
